...
### One-Time Supabase SQL Setup (Copy/Paste)

Run this once in Supabase SQL Editor to bootstrap/align all required tables (`telemetry`, `status`, `device_commands`) and policies in a single pass:
...
create table if not exists public.telemetry (
  ...
  published_uptime_ms bigint,
  captured_boot_id bigint,
  published_boot_id bigint,
  captured_unix_ms bigint,
...
alter table public.telemetry add column if not exists created_at timestamptz not null default now();
alter table public.telemetry add column if not exists device_id text not null default 'unknown';
alter table public.telemetry add column if not exists seq bigint;
alter table public.telemetry add column if not exists uptime_ms bigint;
alter table public.telemetry add column if not exists captured_uptime_ms bigint;
alter table public.telemetry add column if not exists published_uptime_ms bigint;
alter table public.telemetry add column if not exists captured_boot_id bigint;
alter table public.telemetry add column if not exists published_boot_id bigint;
alter table public.telemetry add column if not exists captured_unix_ms bigint;
alter table public.telemetry add column if not exists was_cached boolean not null default false;
alter table public.telemetry add column if not exists temperature_c double precision;
alter table public.telemetry add column if not exists humidity_pct double precision;
alter table public.telemetry add column if not exists pressure_hpa double precision;
alter table public.telemetry add column if not exists battery_v double precision;
alter table public.telemetry add column if not exists sensor_ok boolean;
...
create index if not exists idx_device_commands_device_processed_id
  on public.device_commands (device_id, processed, id asc);
create index if not exists idx_device_commands_created_at
  on public.device_commands (created_at desc);

-- RLS (bring-up profile: permissive for anon + authenticated)
alter table public.telemetry enable row level security;
alter table public.status enable row level security;
alter table public.device_commands enable row level security;

drop policy if exists telemetry_insert_anon on public.telemetry;
create policy telemetry_insert_anon
  on public.telemetry for insert
  to anon, authenticated
  with check (true);

drop policy if exists telemetry_select_anon on public.telemetry;
create policy telemetry_select_anon
...
drop policy if exists commands_select_anon on public.device_commands;
create policy commands_select_anon
  on public.device_commands for select
  to anon, authenticated
  using (true);

drop policy if exists commands_update_anon on public.device_commands;
create policy commands_update_anon
  on public.device_commands for update
  to anon, authenticated
  using (true)
  with check (true);

drop policy if exists commands_insert_anon on public.device_commands;
create policy commands_insert_anon
...
Example status metadata payloads emitted by firmware:
